...
public class MagicMixinPlugin extends EmptyMixinPlugin {
    ...
    public static void remapClass(ClassNode classNode) {
        ...
        Set<MethodNode> remappedMethodNode = new HashSet<>();
        ...
        for (MethodNode methodNode : classNode.methods) {
            ...
            if (remapAnnotation != null) {
                remappedMethodNode.add(methodNode);
            ...
            for (AbstractInsnNode abstractInsnNode : methodNode.instructions) {
                if (abstractInsnNode.getType() == AbstractInsnNode.FIELD_INSN) {
                    ...
                    remapFields.computeIfPresent(fieldInsnNode.name, (namedName, intermediaryName) -> fieldInsnNode.name = intermediaryName);
                }
                if (abstractInsnNode.getType() == AbstractInsnNode.METHOD_INSN) {
                    MethodInsnNode methodInsnNode = (MethodInsnNode) abstractInsnNode;
                }
        ...
        classNode.methods.removeIf(methodNode -> containsMethodNode(remappedMethodNode, methodNode.name, methodNode.desc));
        classNode.methods.addAll(remappedMethodNode);
    }
