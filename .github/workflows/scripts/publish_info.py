...
def main():
    ...
    for subproject, subproject_data in settings['projects'].items():
        for version in subproject_data['versions']:
            module: Module = Module.of(version)

            if module.mc_ver() != mc_ver or module.platform() != platform:
                continue

            proj_path = f'{subproject}/versions/{version}/gradle.properties'
