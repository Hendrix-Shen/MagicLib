...
def get_projects_by_platform() -> Dict[str, List[str]]:
    ...
    for project in settings['projects']:
        for version in settings['projects'][project]['versions']:
            module: Module = Module.of(version)
            platform: str = module.platform()

            if platform not in projects:
                projects[platform] = []

            projects[platform].append(module.mc_ver())

    projects = {
        platform: sorted({item for item in projects[platform]})
        for platform in projects if projects[platform]
    }
